...
out_scores = os.path.join(home_dir, 'classifier_outputs', '200914', 'all_trials_pca_%d_comps_double_odour_accs.npy' % int(list(sys.argv)[1]))
out_predicts = os.path.join(home_dir, 'classifier_outputs', '200914', 'all_trials_pca_%d_comps_double_odour_preds.npy' % int(list(sys.argv)[1]))
...
for j in tqdm(range(100), leave=False):
    ...
    for i in range(32):
        xs, resp1, saved_trial1 = recs.get_binned_trial_response('%d_1' % i, pre_trial_window=0, post_trial_window=.88, w_bootstrap=True, bootstrap_limit=1000, saved_trial=True)
        xs, resp2, saved_trial2 = recs.get_binned_trial_response('%d_3' % i, pre_trial_window=0, post_trial_window=.88, w_bootstrap=True, bootstrap_limit=1000, saved_trial=True)
        saved_trial = np.append(np.concatenate(saved_trial1), np.concatenate(saved_trial2), axis=0)
        resp = np.append(resp1, resp2, axis=1)
        saved_trials.append(saved_trial)
        resps.append(resp)
    resps = np.array(resps)
    saved_trials = np.array(saved_trials)
    reshaped_resps = np.rollaxis(resps, axis=2).reshape(290, 32*1000*100)
    reshaped_saved = np.rollaxis(saved_trials, axis=1).reshape(290, 32*100)
    scaler = StandardScaler()
    scaled_resps = scaler.fit_transform(reshaped_resps.T).T
    scaled_saved = scaler.transform(reshaped_saved.T).T
    reshaped_scaled = scaled_resps.reshape(290, 32, 1000, 100)
    reshaped_scaled = reshaped_scaled.reshape(290*32*1000, 100)

    reshaped_saved = scaled_saved.reshape(290, 32, 100)
    reshaped_saved = reshaped_saved.reshape(290*32, 100)

    pca = PCA(n_components=comps)
    pcad_reshaped = pca.fit_transform(reshaped_scaled)
    pcad_saved_reshaped = pca.transform(reshaped_saved)
    pcad_resps = pcad_reshaped.reshape(290, 32, 1000, comps)
    pcad_saved = pcad_saved_reshaped.reshape(290, 32, comps)
