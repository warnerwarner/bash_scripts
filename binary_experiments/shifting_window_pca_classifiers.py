...
pre_trial_window = int(list(sys.argv)[1])/100
post_trial_window = round(1-pre_trial_window - 0.12, ndigits=3)
out_scores = os.path.join(home_dir, 'classifier_outputs', '200916', 'all_trials_odour2_pca_shifted_window_%d_accs.npy' % int(list(sys.argv)[1]))
out_predicts = os.path.join(home_dir, 'classifier_outputs', '200916', 'all_trials_odour2_pca_shifted_window_%d_preds.npy' % int(list(sys.argv)[1]))
...
for j in tqdm(range(100), leave=False):
    resps = []
    saved_trials = []
    trial_names = ['%d_3' % i for i in range(32)]
    for i in trial_names:
        xs, resp, saved_trial = recs.get_binned_trial_response(i, pre_trial_window=pre_trial_window, post_trial_window=post_trial_window, w_bootstrap=True, bootstrap_limit=1000, saved_trial=True)
    ...
    pcad_resps = pcad_reshaped.reshape(145, 32, 1000, 100)
    pcad_saved = pcad_saved_reshaped.reshape(145, 32, 100)
