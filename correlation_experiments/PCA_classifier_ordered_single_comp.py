...
# Set the number of components for the classifier, by default this trace will have 599 time points (6s * 100 bins/s - 1)
n_components = 599
...
if len(out_lines) == n_components + 1 - len(comp_index):
    ...
    if len(comp_index) != n_components:
        ...
        temp_bash_dir = os.path.join(temp_dir, 'PCA_temp_bash_%d.sh' % len(comp_index))
